package uk.ac.ic.doc.biomedia.plugins.userattributes.rest;
...
public class UserAttributeRestController extends AbstractXapiRestController {
    protected UserAttributeRestController(UserManagementServiceI userManagementService, RoleHolder roleHolder) {
    ...
    public ResponseEntity<UserAttributes> createEntity(@RequestBody final UserAttributes entity) {
        final UserAttributes created = _service.create(entity);
    ...
    public ResponseEntity<Void> updateEntity(@PathVariable final Long id, @RequestBody final UserAttributes entity) {
        final UserAttributes existing = _service.retrieve(id);
    ...
    @Autowired
    private UserAttributesService _service;
}
